...
def main(argv):
    ...
    ## delete afterwards
    if FLAGS.wandb:
        #wandb.init(project=FLAGS.wandb_project, name=f'method{arguments['prune_method']}dataset{arguments['dataset']}sparsity{arguments['sparsity_ratio']}seed{arguments['seed']}model{arguments['model']}')
        wandb.init(project=FLAGS.wandb_project, name=f'method{arguments["prune_method"]}dataset{arguments["dataset"]}sparsity{arguments["sparsity_ratio"]}seed{arguments["seed"]}model{arguments["model"]}H1{arguments["H1"]}H2{arguments["H2"]}')
...
if __name__ == '__main__':
    ...
    # HAM hyperparams
    flags.DEFINE_bool('HAM', False, 'Hyperbolic Aware step')
    flags.DEFINE_float('H1', 200, 'H1 for HAM')
    flags.DEFINE_float('H2', 1e-3, 'H1 for HAM')
